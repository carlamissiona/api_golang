...
func data_todb(db * db.Adapter) {
        ...
        var sqlstring = "INSERT INTO covid_observations (sno,   observationdate,  provincestate,  countryRegion,   lastupdate, confirmed, deaths, recovered )  VALUES "
    ...
    if err != nil {
        log.Printf("=>ERROR!! NO CSV FILE \n %v \n=>END OF ERROR!! NO CSV FILE , %v ", err, csvfile)
            ...
            // ports.handle_error_csv_file() pkgs.postgresql
    } else { 
        ...
        for {
            ...
            each_record, err:= csv_reader.Read(); str := strings.Join(each_record, " ");
            log.Println( str );
            log.Println( strings.Contains(str, "sNo") )  
            if (strings.Contains(strings.Join(each_record, " "), "sNo")) {
                log.Println(each_record);
                log.Println("each_record has SNO SNO has Sno ");
  ...
  str :=  "VALUES ( SNo,  timestamp 'ObservationDate',  'ProvinceState',  'CountryRegion',  timestamp 'Last Update', Confirmed, Deaths, Recovered ) , "

   str_replace :=  "VALUES  " 
 
    log.Println(strings.ReplaceAll(sqlstring, str, str_replace  ))
    st := strings.ReplaceAll(sqlstring, str, str_replace)
    log.Println(st)
  log.Println("st")
  log.Println(st)

log.Println(strings.Index(st,str_replace ))
...
func retry_csv() {
    ...
    // db.Execute(`INSERT INTO covid_observations (sno,observationdate, provincestate,countryregion,lastupdate, confirmed,deaths,recovered) VALUES(0,'Mainland China','Anhui',102,10,2,timestamp '2015-01-10 17:00:00',timestamp '2015-01-10 17:00:00' ) `)